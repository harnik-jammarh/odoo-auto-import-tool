...
// account be?" pre-flight prompt (see the isAccountField check inside
// uploadSheetToOdoo) — same keys as ACCOUNT_CODE_BASE, in the order Odoo's
// own Chart of Accounts form groups them (Assets, Liabilities, Equity,
// Income, Expenses).
...
// ---------------------------------------------------------------------------
// Shared by parseWorkbookFile (a real uploaded .xlsx/.xls/.csv) and the
// Google Sheets import path (a fetched sheet, read as CSV text) — turns a
...
// analysis either path goes through equally. This is a pure extraction of
// what used to live inline inside parseWorkbookFile — no behavior change.
export function buildSheetsFromWorkbook(wb, embeddedBySheet = {}) {
  return wb.SheetNames.map((name) => {
    ...
    // Drop fully-blank trailing/stray rows so they don't get counted,
    // previewed, or uploaded as empty junk records.
...
// same buildSheetsFromWorkbook pipeline as an uploaded file. The sheet's
// sharing must be set to "Anyone with the link" (Viewer is enough) — a
// private sheet's export endpoint redirects to a Google sign-in page
// instead of CSV data, which is caught and reported as a clear error
// rather than a silent empty/garbled import.
//
// Limitation (same as the extension): Google's export endpoint returns
// ONE tab per request (the gid in the URL, or the first/default tab if no
// gid is present) — a multi-tab workbook needs its tabs imported one at a
// time, by pasting each tab's own URL (click the tab in Google Sheets
// first, then copy the address bar — its gid changes per tab).
//
// The actual fetch of docs.google.com happens server-side, via
// /api/sheet-proxy — a plain client fetch() from this app's own domain to
// docs.google.com is blocked by CORS (that endpoint doesn't send back an
// Access-Control-Allow-Origin header for arbitrary third-party origins),
// the same class of problem /api/image-proxy already solves for Drive/
// product-image URLs. See that route's comment header for the full
// reasoning.
// ---------------------------------------------------------------------------
...
export function extractGoogleSheetGid(url) {
  const m = String(url || "").match(/[?#&]gid=(\d+)/);
  return m ? m[1] : null;
}

// Takes a Google Sheets URL, resolves it through the server-side proxy, and
// returns { sheets, fileName } on success. Throws an Error with a
// human-readable message (safe to show directly in the UI) on failure.
export async function importGoogleSheet(url) {
  ...
  if (!res.ok || data.error) {
    throw new Error(data.error || `couldn't fetch this sheet (HTTP ${res.status}) — make sure it's shared as "Anyone with the link" (Viewer access is enough), not just to specific people.`);
  }

  const csvText = data.csv || "";
  // A private (or link-restricted) sheet's export endpoint 302s to a
  // Google accounts sign-in page rather than erroring outright — the proxy
  // already checks for this server-side (HTTP status + final redirected
  // URL), but the body itself is sniffed again here in case an HTML page
  // slipped through some other way.
  ...
  const sheets = buildSheetsFromWorkbook(wb, {});
  if (!sheets.length) {
    throw new Error("the sheet came back empty, or only has a header row with no data below it — check you copied the right tab's link.");
  }
  return { sheets, fileName: `Google Sheet (${gid ? `tab gid=${gid}` : "first tab"})` };
}
...
export function createEngine(connection) {
  ...
  // pre-flight prompt (see the isAccountField check inside
  // uploadSheetToOdoo). When given, this is used verbatim instead of
  // classifyAccountType's keyword guess. Only consulted on the create path
  // — a name that already matches an existing account is reused as-is
  // either way.
  ...
  async function buildInvoiceLikeGroup(group, mappedFields, schema, notes) {
    ...
    return { values, linesByField, pendingPoRef, pendingPoName, pendingTdsSection, pendingTdsBase, pendingTdsRate, pendingExtraLine, pendingOriginRef };
  }
  ...
  // Sales Invoices module: applies the optional TDS-at-import columns to a
  // Customer Invoice right after it's created (or updated). Mirrors the TDS
  // half of applyVendorBillTdsAndExtras above, but always resolves the
  // "sales" side. Ported from popup.js ~2620-2633.
  async function applyMoveTdsOnly(moveId, tds, side, notes, errors, rowIndex) {
    if (!tds || !tds.section) return;
    try {
      ...
      if (result) notes.push(`"${move.name}": TDS Entry posted via "${result.taxName}" — ${result.amount}.`);
    } catch (e) {
      errors.push({ row: rowIndex, error: `TDS step failed for move #${moveId}: ${e.message}`, data: {} });
    }
  }
  ...
  async function processSalesInvoicesSheet(sheet, onProgress) {
    ...
    for (let g = 0; g < groups.length; g++) {
      try {
        const { values, linesByField, pendingPoRef, pendingPoName, pendingTdsSection, pendingTdsBase, pendingTdsRate } =
          await buildInvoiceLikeGroup(groups[g], mappedFields, schema, notes);
        ...
        await applyMoveTdsOnly(upsert.id, { section: pendingTdsSection, base: pendingTdsBase, rate: pendingTdsRate }, "sales", notes, errors, g);
      } catch (e) {
        errors.push({ row: g, error: e.message, data: groups[g][0] });
      }
      emit({ status: "uploading", progress: g + 1, total: groups.length });
  ...
  async function uploadSheetToOdoo(sheet, onProgress) {
    ...
    // keywords, ask once per sheet — same "stop and show a list before
    // creating anything" pattern as the confirmFields check above, just
    // with a type picker per name instead of a plain yes/no.
    // sheet.accountTypeChoices, once set (even to {} when nothing needed
    // asking), means this sheet has already been through this check —
    // skip re-asking on a retry/re-click.
